...
// Map steps to data keys
const STEP_KEYS = ["name", "location", "seats", "budget"];

// --- SMART MAPPING: Maps Typos & Areas to MAIN CITY ---
const CITY_MAPPING = {
  // Typos
  "banglore": "bangalore",
  "bengaluru": "bangalore",
  ...
  // Micro-markets (Areas) mapped to Main City
...
const ChatBot = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (currentStep < STEP_KEYS.length) {
        ...
        leadDataRef.current[key] = userText;
    }
    ...
    setTimeout(() => {
      ...
      if (currentStep < questions.length - 1) {
        ...
        // Check our mapping list. If found (e.g., 'indiranagar'), use 'bangalore'. 
        // If not found, use the raw input (fallback).
        ...
        // --- REDIRECT (Ignore seats/budget/micro-market) ---
        setTimeout(() => {
            const params = new URLSearchParams();
            // ONLY append the Main City location
            if (mainCity) {
                params.append("location", mainCity);
            }
    ...
    <div className="w-full max-w-md mx-auto relative z-20">
      ...
      {!isOpen && (
        <div 
          ...
          className="bg-white/90 backdrop-blur-md p-3 rounded-full shadow-xl border border-gray-200 cursor-pointer hover:scale-[1.02] transition-all duration-300 group flex items-center justify-between gap-4 animate-in fade-in zoom-in-95"
        >
           ...
           <div className="w-10 h-10 rounded-full bg-gray-50 group-hover:bg-teal group-hover:text-white text-gray-400 flex items-center justify-center transition-colors">
      ...
      {isOpen && (
        <div className="w-full flex flex-col overflow-hidden shadow-2xl rounded-2xl border border-gray-200 bg-white ring-1 ring-black/5 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div
            ...
            className="h-72 overflow-y-auto p-4 space-y-4 bg-gray-50/50 scroll-smooth"
          >
