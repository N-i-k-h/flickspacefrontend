...
import { MapPin, Users, IndianRupee, ArrowRight, ImageOff, Search, ChevronDown, ChevronUp, Filter, X } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    const exactMatches = data.filter((space) => {
      if (locationParam && space.city.toLowerCase() !== locationParam.toLowerCase()) return false;
      
      if (marketParam) {
        const spaceLoc = space.location.toLowerCase().replace(/[^a-z0-9]/g, '');
        const searchLoc = marketParam.toLowerCase().replace(/[^a-z0-9]/g, '');
    ...
    if (locationParam) {
      suggestions = data.filter(space => {
        const isSameCity = space.city.toLowerCase() === locationParam.toLowerCase();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4 max-w-full xl:max-w-7xl">
            ...
            <div className={`${isFiltersOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row gap-4 items-end`}>
              ...
              <div className="w-full md:w-1/6 space-y-1.5">
                ...
                <Select value={selectedMarket} onValueChange={setSelectedMarket} disabled={!selectedCity}>
                  <SelectTrigger className="h-10 bg-white border-gray-200 focus:ring-teal"><SelectValue placeholder={selectedCity ? "Area" : "Select City"} /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="w-full md:w-1/6 space-y-1.5">
                ...
                <Select value={selectedType} onValueChange={setSelectedType}>
                  <SelectTrigger className="h-10 bg-white border-gray-200 focus:ring-teal"><SelectValue placeholder="All Types" /></SelectTrigger>
                  <SelectContent>
          ...
          {/* Results Header: Navy Heading, Teal Count */}
